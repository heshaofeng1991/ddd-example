...
YAML_FILES=$(find $GOPATH/src/api/http/oms -name "*.yaml")
...
for file in $YAML_FILES; do
  ...
  oapi-codegen -generate gin -o "$INTERFACE_NAME/$filename/http_server.gen.go" -package "$INTERFACE_NAME" "$PROJECT_DIR/$filename.yaml"
